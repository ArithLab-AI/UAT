...
def _is_age_column_name(column_name: str) -> bool:
    ...
    return "age" in token_set or normalized in AGE_COLUMN_HINTS or ("years" in token_set and "old" in token_set)
...
def _parse_age_value(value: Any) -> int | None:
    return _parse_strict_integer_value(value, min_value=AGE_MIN_VALUE, max_value=AGE_MAX_VALUE)


def _parse_datetime_series(values: pd.Series) -> pd.Series:
    try:
        return pd.to_datetime(values, errors="coerce", format="mixed", utc=False)
...
def _compact_column_profile(column_profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {key: column_profile[key] for key in allowed_keys if key in column_profile}
...
def _update_column_stats(stats: dict[str, Any], series: pd.Series) -> None:
    ...
    if not valid_phone_values.empty:
        ...
        _append_examples(stats["phone_format_examples"], valid_phone_values[phone_format_changes].tolist())
    ...
    if not valid_numeric_values.empty:
        ...
        _append_examples(stats["numeric_format_examples"], valid_numeric_values[numeric_format_changes].tolist())
    ...
    if not valid_age_values.empty:
        ...
        _append_examples(stats["age_format_examples"], valid_age_values[age_format_changes].tolist())
    ...
    if not valid_date_values.empty:
        formatted_date_values = parsed_dates[date_matches].dt.strftime(DATE_OUTPUT_FORMAT).astype("string")
        ...
        _append_examples(stats["date_format_examples"], valid_date_values[date_format_changes].tolist())
    ...
    _append_examples(stats["text_examples"], stripped_non_null[string_series[non_null_mask] != stripped_non_null].tolist())
    _append_examples(stats["semantic_type_mismatch_examples"], stripped_non_null[~text_like_matches].tolist())
...
def build_dataset_profile_from_chunks(chunks: Iterable[pd.DataFrame]) -> dict[str, Any]:
    ...
    for chunk_df in chunks:
        ...
        if chunk_len:
            ...
            row_hashes = pd.util.hash_pandas_object(comparable_df.astype(str), index=False).astype("uint64").tolist()
    ...
    for column_name in column_order:
        ...
        if is_phone_candidate:
            ...
            phone_format_inconsistency_percent = round(float(stats["phone_format_inconsistency_count"]) / non_null_count * 100, 2)
        ...
        if is_date_candidate:
            ...
            date_format_inconsistency_percent = round(float(stats["date_format_inconsistency_count"]) / non_null_count * 100, 2)
        ...
        if is_age_candidate:
            ...
            age_format_inconsistency_percent = round(float(stats["age_format_inconsistency_count"]) / non_null_count * 100, 2)
        ...
        if expected_type == "integer" and not is_age_candidate:
            invalid_integer_percent = round(float(non_null_count - stats["integer_valid_count"]) / row_count_safe * 100, 2)
        ...
        if expected_type == "float":
            invalid_float_percent = round(float(non_null_count - stats["float_valid_count"]) / row_count_safe * 100, 2)
        ...
        if is_numeric_candidate and not is_phone_candidate and not is_age_candidate and expected_type not in {"integer", "float"}:
            ...
            numeric_format_inconsistency_percent = round(float(stats["numeric_format_inconsistency_count"]) / non_null_count * 100, 2)
        ...
        if is_semantic_text_candidate:
            semantic_type_mismatch_percent = round(float(non_null_count - stats["text_like_count"]) / row_count_safe * 100, 2)
        ...
        is_moderate_cardinality = stats["track_uniques"] and unique_count <= 50 and unique_ratio <= 0.5 and average_length <= 80
...
def compute_quality_score(profile: dict[str, Any]) -> int:
    ...
    affected_column_count = int(profile.get("affected_column_count", sum(1 for column in columns if _column_issue_percent(column) > 0.0)))
    affected_column_percent = float(profile.get("affected_column_percent", (affected_column_count / total_columns) * 100.0))
    max_column_issue_percent = float(profile.get("max_column_issue_percent", max((_column_issue_percent(column) for column in columns), default=0.0)))
    dataset_issue_count = len(profile.get("dataset_issues", []))
    consistency_issue_percent = float(profile.get("average_consistency_issue_percent", profile.get("average_text_inconsistency_percent", 0.0)))
...
def _columns_with_metric(columns: list[dict[str, Any]], metric_name: str, *, threshold: float = 0.0) -> list[str]:
    return [
        str(column.get("name"))
        for column in columns
        if str(column.get("name", "")).strip() and float(column.get(metric_name, 0.0)) > threshold
    ]
...
def generate_rule_based_suggestions(profile: dict[str, Any], *, max_suggestions: int = 10) -> list[DataSuggestion]:
    ...
    for boolean_targets in _group_column_targets(boolean_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Boolean-like column(s) {boolean_targets} contain values outside the allowed set True, False, 1, or 0.",
                priority="High",
                resolution_prompt=(
                    f"Only in column(s) {boolean_targets}, accept only True, False, 1, and 0 case-insensitively, "
                    f"replace every other value with '{NULL_OUTPUT_TOKEN}', and leave unrelated columns untouched."
                ),
                cleaning_prompt_type="boolean_validation",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", boolean_targets)],
            )
        )
    ...
    for phone_targets in _group_column_targets(phone_issue_columns):
        suggestions.append(
            DataSuggestion(
                ...
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", phone_targets)],
            )
        )
    ...
    for date_targets in _group_column_targets(date_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Date-like column(s) {date_targets} contain invalid values or inconsistent date formatting.",
                priority="High",
                resolution_prompt=(
                    f"Only in column(s) {date_targets}, parse values with pandas.to_datetime(errors='coerce'), "
                    f"keep parseable dates, replace invalid dates with '{NULL_OUTPUT_TOKEN}', "
                    f"and format valid dates to {DATE_OUTPUT_FORMAT} without modifying non-date columns."
                ),
                cleaning_prompt_type="date_normalization",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", date_targets)],
            )
        )
    ...
    for missing_targets in _group_column_targets(missing_value_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Column(s) {missing_targets} contain a meaningful share of missing, blank, or placeholder values.",
                priority="Medium",
                resolution_prompt=(
                    f"Only in column(s) {missing_targets}, normalize blank, null-like, and placeholder values to "
                    f"the single token '{NULL_OUTPUT_TOKEN}', replace only genuinely missing entries, and do not invent or infer missing data."
                ),
                cleaning_prompt_type="missing_value_normalization",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", missing_targets)],
            )
        )
    ...
    integer_issue_columns = [
        str(column.get("name"))
        for column in columns
        if float(column.get("invalid_integer_percent", 0.0)) > 0.0 and float(column.get("invalid_age_percent", 0.0)) == 0.0
    ]
    for integer_targets in _group_column_targets(integer_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Integer-like column(s) {integer_targets} contain non-integer values under strict validation.",
                priority="High",
                resolution_prompt=(
                    f"Only in column(s) {integer_targets}, keep only valid integers in strict mode, "
                    f"replace non-integer values with '{NULL_OUTPUT_TOKEN}', and leave unrelated columns untouched."
                ),
                cleaning_prompt_type="integer_validation",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", integer_targets)],
            )
        )
    ...
    for float_targets in _group_column_targets(float_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Float-like column(s) {float_targets} contain values that are not valid floats.",
                priority="High",
                resolution_prompt=(
                    f"Only in column(s) {float_targets}, keep only valid float values, replace every other value with '{NULL_OUTPUT_TOKEN}', "
                    "and leave unrelated columns untouched."
                ),
                cleaning_prompt_type="float_validation",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", float_targets)],
            )
        )
    ...
    for age_targets in _group_column_targets(age_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Age-like column(s) {age_targets} contain non-integer, negative, impossible, or inconsistent age values.",
                priority="High",
                resolution_prompt=(
                    f"Only in column(s) {age_targets}, keep only strict integer ages between {AGE_MIN_VALUE} and {AGE_MAX_VALUE}, "
                    f"replace negative, impossible, or non-integer age values with '{NULL_OUTPUT_TOKEN}', and leave unrelated columns untouched."
                ),
                cleaning_prompt_type="age_normalization",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", age_targets)],
            )
        )
    ...
    for semantic_targets in _group_column_targets(semantic_type_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Text-like column(s) {semantic_targets} contain values that do not match the header's expected text/string type.",
                priority="High",
                resolution_prompt=(
                    ...
                    f"and replace values that are only numbers or obvious non-text placeholders with '{NULL_OUTPUT_TOKEN}'. Do not modify unrelated columns."
                ),
                cleaning_prompt_type="header_type_normalization",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", semantic_targets)],
            )
        )
    ...
    for text_targets in _group_column_targets(text_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Moderate-cardinality text column(s) {text_targets} show casing or whitespace inconsistencies.",
                priority="Medium",
                resolution_prompt=(
                    f"Only in column(s) {text_targets}, trim leading and trailing whitespace and standardize repeated text values "
                    "that clearly represent the same meaning to one canonical casing. Preserve free-text content and do not rewrite unrelated long-form text."
                ),
                cleaning_prompt_type="text_normalization",
                target_columns=[column.strip("'") for column in re.findall(r"'([^']+)'", text_targets)],
            )
        )
    ...
    for numeric_targets in _group_column_targets(numeric_issue_columns):
        suggestions.append(
            DataSuggestion(
                issue_description=f"Numeric-like column(s) {numeric_targets} contain formatting noise or inconsistent numeric presentation.",
                priority="Medium",
                resolution_prompt=(
