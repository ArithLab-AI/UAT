...
def ensure_upload_storage_available(
    ...
    *,
    ...
    used_file_size_bytes = get_used_upload_storage_bytes(db, user_id)
    remaining_file_size_bytes = max(total_file_size_bytes - used_file_size_bytes, 0)
    if upload_size_bytes > remaining_file_size_bytes:
        raise error_response(
            status_code=400,
            detail=(
                f"Your current plan has {_format_bytes(remaining_file_size_bytes)} upload storage remaining. "
                "Please upgrade your plan."
            ),
        )


def get_user_uploaded_dataset_usage_count(db: Session, user_id: int) -> int:
    sync_user_upload_storage_usage(db, user_id)
    return (
        db.query(func.count(UserUploadStorageUsage.id))
        .filter(UserUploadStorageUsage.user_id == user_id)
        .scalar()
    ) or 0


def ensure_upload_count_available(
    db: Session,
    *,
    user_id: int,
    plan_capabilities: dict,
    ...
    max_uploaded_datasets = plan_capabilities.get("max_uploaded_datasets")
    if max_uploaded_datasets is None:
        return

    used_upload_count = get_user_uploaded_dataset_usage_count(db, user_id)
    if used_upload_count + upload_count > max_uploaded_datasets:
        raise error_response(
            status_code=400,
            detail=(
                f"Your current plan allows uploading up to {max_uploaded_datasets} datasets. "
                "Please upgrade your plan."
            ),
        )
    ...
    *,
    ...
    existing_usage = (
        ...
        .filter(
            ...
            UserUploadStorageUsage.uploaded_dataset_id == dataset.id,
        )
        .first()
    )
    if existing_usage:
