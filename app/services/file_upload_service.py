...
def save_upload_file(upload_file: UploadFile) -> tuple[str, str, str]:
    """Save the uploaded file and return the file_id, storage key, and object storage URL."""
    ...
    if not storage_service.enabled:
        raise ValueError("AWS S3 bucket is not configured for file uploads.")

    job_id = str(uuid.uuid4())
    fd, file_path = tempfile.mkstemp(prefix=f"{job_id}_", suffix=".csv")
    os.close(fd)

    storage_key = raw_upload_key(job_id)
    ...
    try:
        ...
        file_url = storage_service.upload_file(file_path, storage_key)
    finally:
        if os.path.exists(file_path):
            os.remove(file_path)

    return job_id, storage_key, file_url
